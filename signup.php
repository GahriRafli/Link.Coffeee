...
<div class="container-fluid">
	<form action="" id="signup-frm">
		...
		<div class="form-group">
			<label for="" class="control-label"> No Kontak</label>
	...
	$('#signup-frm').submit(function(e){
		...
		$('#signup-frm button[type="submit"]').removeAttr('disabled').html('Create');
			...
			success:function(resp){
				if(resp == 1){
					location.href ='<?php echo isset($_GET['redirect']) ? $_GET['redirect'] : 'login.php' ?>';
				}else{
					$('#signup-frm').prepend('<div class="alert alert-danger">Email Sudah Ada.</div>')
					$('#signup-frm button[type="submit"]').removeAttr('disabled').html('Create');
				}
			}
		})
